...
protected function _enddoc()
{
	...
	function myCell($w, $h, $x, $texto)
    {
        $height = $h / 3;
        $first = $height + 2;
        $second = $height + $height + $height + 3;
        $tercer = $height + $height + $height + $height + $height + 4;
        $len = strlen($texto);
        if ($len > 23 && $len < 46) {
            $txt = str_split($texto, 23);
            $this->SetX($x);
			$this->Cell($w, $first, $txt[0], 0, 0, 'L');

            $this->SetX($x);
            $this->Cell($w, $second, $txt[1], 0, 0, 'L');
            ...
            $this->SetX($x);
            $this->Cell($w, $tercer, $txt[2], 0, 0, 'L');

			$this->SetX($x);
            $this->Cell($w, $h, '', 0);
        } else {
            $this->SetX($x);
            $this->Cell($w, $h, $texto, 0);
        }
    }
	function comentarios($w, $h, $x, $texto)
    {
        $height = $h / 3;
        $first = $height + 2;
        $second = $height + $height + $height + 3;
